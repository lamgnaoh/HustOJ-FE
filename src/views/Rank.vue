  ...
  <div>
    <Row>
      <Col span="20" offset="2">
        <div class="my-info">
          <p>
            My data: Number of problems passed: {{ acCount }} Total number of submissions:
            {{ submitCount }}
          </p>
        </div>
      </Col>
    ...
    <Row>
      <Col span="20" offset="2">
        <Row
            class="list"
            style="color: #17233d;font-size: 14px;font-weight: 500"
        >
          <Col span="1">
          ...
          <Col span="4" offset="1">
            <div class="list-title">
              ...
              ><p style="color: #515a6e">Username</p></router-link
              >
            </div>
          </Col>
        ...
        <Row v-for="(item, index) in ranks" class="list" :key="index">
          ...
          <Col span="4" offset="1">
            <div class="list-title">
              <a style="color: #2d8cf0" @click="toPersonal(item.id)">{{
                  item.username
                }}</a>
            </div>
          </Col>
          <Col span="10" align="left">
            <p>{{ item.sign }}</p>
            <p v-if="item.sign === null">None yet</p>
          </Col>
          <Col span="3">
            <p>{{ item.ac }}</p>
          </Col>
    ...
    .then((res: any) => {
      ...
      res.data.list.forEach(function(item: any) {
        that.ranks.push({
          ...
          rate:
              item.submitCount !== 0
                  ? Math.floor((item.acCount / item.submitCount) * 100) + '%'
                  : '0%',
        })
      })
    })
